...
use lib "$Bin/";
use Lastz;
die "tab.alignment_conver.tab , (optional: repeat.gff gene.structure.gffs zebrafish.gff self.gff),  prefix\n" unless @ARGV>=2;
#################################################
my ($S1,$T1,$S2,$T2);
my %tab;
Lastz::readTab(\%tab,$ARGV[0]);
my %repeat;
Lastz::readGFF(\%repeat,$ARGV[1]) if $ARGV[1] =~ /gff/;
my %gff;
Lastz::readGFF(\%gff,$ARGV[2]) if $ARGV[2] =~ /gff/;
#my %sgff;
